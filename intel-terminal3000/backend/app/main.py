...
# Create FastAPI app
app = FastAPI(
    title="Intel Terminal 3000",
    description="Unified intelligence command platform (feeds + map + wallboard + video)",
    version="3.0.0",
    lifespan=lifespan,
)
...
async def get_articles(
    ...
    if severity:
        # Support comma-separated severity values (e.g., "critical,high")
        severity_list = [s.strip() for s in severity.split(',')]
        if len(severity_list) > 1:
            filters.append(Article.severity.in_(severity_list))
        else:
...
async def get_categories(db: AsyncSession = Depends(get_db)):
    """Get all known categories from sources and articles"""
    source_result = await db.execute(
        select(Source.category)
        .where(Source.category.isnot(None))
    ...
    categories = sorted({
        *(row[0] for row in source_result.all() if row[0]),
        *(row[0] for row in article_result.all() if row[0]),
    })
...
async def create_category(category_data: Dict[str, Any], db: AsyncSession = Depends(get_db)):
    ...
    existing = await db.execute(select(Source).where(Source.category == name).limit(1))
    if existing.scalar_one_or_none():
        return {"status": "exists", "name": name}

    source = Source(
        name=f"Category Seed: {name}",
        source_type="rss",
        url="https://example.com/placeholder.xml",
        category=name,
        enabled=False,
        config={"category_seed": True},
    )
    db.add(source)
    await db.commit()
...
async def delete_category(category_name: str, db: AsyncSession = Depends(get_db)):
    ...
    sources_result = await db.execute(select(Source).where(Source.category == category_name))
    sources = sources_result.scalars().all()
    for source in sources:
        if source.config and source.config.get("category_seed"):
            await db.delete(source)
        else:
            source.category = None

    articles_result = await db.execute(select(Article).where(Article.category == category_name))
...
async def get_heatmap_data(
    ...
    result = await db.execute(query)
    articles = result.scalars().all()
    # Aggregate by region and hotspot clusters for pulsing/blinking markers
    region_counts = {}
    hotspot_buckets: Dict[str, Dict[str, Any]] = {}
    points = []
    for article in articles:
        ...
        area_key = (article.country or article.region or "Unknown").strip()
        if area_key not in hotspot_buckets:
            hotspot_buckets[area_key] = {
                "area": area_key,
                "count": 0,
                "lat_sum": 0.0,
                "lon_sum": 0.0,
        ...
        hotspot_buckets[area_key]["count"] += 1
        hotspot_buckets[area_key]["lat_sum"] += float(article.latitude or 0.0)
        hotspot_buckets[area_key]["lon_sum"] += float(article.longitude or 0.0)
        hotspot_buckets[area_key]["max_intensity"] = max(hotspot_buckets[area_key]["max_intensity"], intensity)

    hotspots = []
    for area, item in hotspot_buckets.items():
        count = item["count"]
        if count <= 0:
            continue
        avg_lat = item["lat_sum"] / count
        avg_lon = item["lon_sum"] / count
        hotspot_score = round((count * 0.7) + (item["max_intensity"] * 10), 2)
        hotspots.append({
            "area": area,
            "lat": avg_lat,
            "lon": avg_lon,
...
async def search_aircraft(
    q: str = Query("", description="Search by callsign, ICAO hex, or squawk code"),
    region: str = Query("global", description="Region to search"),
    db: AsyncSession = Depends(get_db)
):
    """Search for aircraft by callsign, ICAO hex code, or squawk - LIVE data"""
    if not q or len(q) < 2:
        return {"error": "Search query too short (minimum 2 characters)", "results": []}
    ...
    try:
        all_aircraft = await collector.fetch(mock_source)
        await collector.close()
        ...
        results = [
            {
                ...
                "heading": a.get("heading"),
                "squawk": a.get("squawk"),
                "aircraft_type": a.get("aircraft_type", "Unknown"),
                "title": a.get("title", ""),
                "link": a.get("link", ""),
            }
            for a in all_aircraft
            if (
                q_lower in (a.get("callsign", "") or "").lower()
                or q_lower in (a.get("icao", "") or "").lower()
                or q_lower in str(a.get("squawk", "") or "")
            )
        ]
...
async def watch_aircraft(
    ...
    callsign = request_data.get("callsign", "").upper()
    squawk = request_data.get("squawk", "")
    alert_enabled = request_data.get("alert_enabled", True)
    
    if not callsign and not squawk:
        return {"error": "Either callsign or squawk required"}
    
    try:
        # Store aircraft watch in database
        from .models import AircraftTrack
        
        watch_entry = AircraftTrack(
            callsign=callsign if callsign else None,
            squawk=squawk if squawk else None,
            is_interesting=True,
            notes=f"User watched: {request_data.get('reason', 'Manual watch')}"
        )
        
        db.add(watch_entry)
        await db.commit()
        await db.refresh(watch_entry)
        ...
        return {
            ...
            "message": "Aircraft added to watch list"
        }
    except Exception as e:
        await db.rollback()
        logger.error(f"Error saving aircraft watch: {e}")
        return {"error": str(e), "status": "error"}


@app.get("/api/marine/interesting")
async def get_interesting_marine(
    region: str = Query("global", description="Region: global, north-atlantic, south-pacific, indian-ocean"),
    db: AsyncSession = Depends(get_db)
):
    """Get maritime traffic of interest - Articles from maritime sources"""
    try:
        stmt = select(Article).where(
            Article.category == "marine"
        ...
        logger.error(f"Error fetching marine articles: {e}")
        return []
...
DASHBOARD_TEMPLATES = {
    "rss_feeder": {
        "name": "RSS Feeder",
        "description": "Default merged RSS dashboard with cards/IRC switch + map link",
        "panels": 4,
        "layout": "2x2",
        ...
        "panel_defs": [
            {"module": "feed", "title": "Live Feed", "filters": {}, "feedView": "tiled"},
            {"module": "feed", "title": "IRC Feed", "filters": {}, "feedView": "irc"},
            {"module": "map", "title": "Story Heat Map", "filters": {}},
            {"module": "stats", "title": "Stats", "filters": {}},
        ],
    },
    "command_center": {
        "name": "Command Center Wallboard",
        "description": "Primary wallboard view for operations",
        "panels": 6,
        "layout": "3x2",
        "categories": ["", "", "map", "trending", "alerts", "aircraft"],
        "keywords": [],
        "region_focus": {"lat": 25.0, "lon": 0.0, "zoom": 2},
        "panel_defs": [
            {"module": "feed", "title": "Priority Feed", "filters": {"severity": "high"}, "feedView": "tiled"},
            {"module": "feed", "title": "IRC Wire", "filters": {}, "feedView": "irc"},
            {"module": "map", "title": "Hot Regions Map", "filters": {}},
            {"module": "trending", "title": "Trending Spikes", "filters": {}},
            {"module": "alerts", "title": "Critical Alerts", "filters": {}},
            {"module": "aircraft", "title": "Air Ops", "filters": {}},
        ],
    },
    "hot_videos": {
        "name": "Hot Video Sources",
        "description": "Video intelligence page for active events",
        "panels": 2,
        "layout": "2x1",
        "categories": ["", ""],
        "keywords": [],
        "region_focus": {"lat": 20.0, "lon": 0.0, "zoom": 2},
        "panel_defs": [
            {"module": "videos", "title": "Live Video Sources", "filters": {}},
            {"module": "web", "title": "Web Module", "filters": {}, "url": "https://www.adsbexchange.com/"},
        ],
    },
    "ukraine_conflict": {
...
async def create_dashboard_from_template(
    ...
    panel_defs = template.get("panel_defs")
    if not panel_defs:
        panel_defs = []
        categories = template.get("categories", [])
        for idx, cat in enumerate(categories):
            module = "feed"
            if str(cat).lower() in {"map", "heatmap"}:
                module = "map"
            elif str(cat).lower() == "trending":
                module = "trending"
            elif str(cat).lower() == "aircraft":
                module = "aircraft"
            elif str(cat).lower() == "stats":
                module = "stats"
            panel_defs.append({
                "id": f"panel-{idx + 1}",
                "module": module,
                "title": str(cat) if cat else f"Panel {idx + 1}",
                "filters": {"category": cat} if module == "feed" and cat else {},
            })

    normalized_panels = []
    for idx, panel in enumerate(panel_defs):
        normalized = dict(panel)
        normalized.setdefault("id", f"panel-{idx + 1}")
