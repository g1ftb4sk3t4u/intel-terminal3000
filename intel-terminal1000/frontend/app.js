...
/**
 * Intel Terminal 1000 - Frontend Application
...
async function initializeApp() {
    console.log('🚀 Intel Terminal 1000 initializing...');
    ...
    // Apply default dashboard or create one
    ...
    renderDashboard();
    console.log('✅ Intel Terminal 1000 ready');
}
...
function createDefaultDashboard() {
    return {
        ...
        layout: '2x2',
        panels: [
            { id: 'panel-1', module: 'feed', title: 'All Intel', filters: {} },
            { id: 'panel-2', module: 'map', title: 'Heat Map', filters: {} },
            { id: 'panel-3', module: 'feed', title: 'Critical', filters: { severity: 'critical' } },
...
function renderDashboard() {
    ...
    // Render panels
    container.innerHTML = dashboard.panels.map(panel => renderPanel(panel)).join('');
...
// Marine-only map panel (placeholder, real API needed)
async function initMarineMapPanel(config) {
    ...
    content.innerHTML = `
        <div style="margin-bottom:8px;">
            <select id="${config.id}-marine-filter" onchange="window.setMarineMapFilter('${config.id}', this.value)">
                <option value="all" ${filter === 'all' ? 'selected' : ''}>All Ships</option>
                <option value="commercial" ${filter === 'commercial' ? 'selected' : ''}>Commercial</option>
                <option value="military" ${filter === 'military' ? 'selected' : ''}>Military</option>
    ...
    // Placeholder: show static ships
    let ships = await fetchMarineShips();
    if (filter === 'commercial') ships = ships.filter(s => s.type === 'Cargo' || s.type === 'Container' || s.type === 'Cruise');
    if (filter === 'military') ships = ships.filter(s => s.type === 'Military');
    if (filter === 'alerts') ships = ships.filter(s => s.alert);
    ships.forEach(s => {
        const marker = L.marker([s.lat, s.lon], {
            icon: L.divIcon({ className: 'marine-marker', html: '🚢', iconSize: [24,24], iconAnchor: [12,12] })
        });
        marker.bindPopup(`<div class=\"map-popup\">${s.name} (${s.type})<br>Dest: ${s.destination}<br>Speed: ${s.speed} kts</div>`);
        marker.addTo(map);
    });
    setTimeout(() => map.invalidateSize(), 100);
...
async function initMarinePanel(config) {
    // TODO: Integrate AIS ship tracking API and render ship locations on map
    const container = document.getElementById(config.id + '-content');
    container.innerHTML = '<div class="loading"><div class="spinner"></div></div>';
    // Example placeholder: fetch and display ships
    try {
        // Replace with actual AIS API call
        const ships = await fetchMarineShips();
        container.innerHTML = ships.length
            ? ships.map(s => `<div>🚢 <strong>${s.name}</strong> (${s.lat}, ${s.lon})</div>`).join('')
            : '<div>No ships found.</div>';
    } catch (e) {
        container.innerHTML = '<div>Error loading ships.</div>';
    }
}

async function fetchMarineShips() {
    // Static sample ship data for demo purposes
    return [
        { name: 'Ever Given', lat: 30.5, lon: 32.3, type: 'Cargo', destination: 'Suez', speed: 12.5 },
        { name: 'Maersk Line', lat: 29.8, lon: 31.2, type: 'Container', destination: 'Port Said', speed: 14.2 },
        { name: 'Queen Mary 2', lat: 36.1, lon: -5.3, type: 'Cruise', destination: 'Southampton', speed: 18.0 },
        { name: 'USS Enterprise', lat: 34.0, lon: -7.0, type: 'Military', destination: 'Norfolk', speed: 20.0, alert: true }
    ];
}
...
function renderFeedIRCContent(panelId, articles) {
    ...
    if (articles.length === 0) {
        content.innerHTML = `<div class="empty-state"><span style="font-size: 3rem;">📭</span><p>No articles found</p></div>`;
...
function renderIRCFeedItem(article) {
    ...
    return `<div class="irc-feed-item">
        <span class="irc-feed-time">[${timeAgo}]</span>
        <span class="irc-feed-title"><a href="${escapeHtml(articleUrl)}" target="_blank" rel="noopener">${escapeHtml(article.title)}</a></span>
        <span class="irc-feed-meta">(${escapeHtml(article.source || '')}${article.region ? ', ' + article.region : ''}, ${article.severity})</span>
    </div>`;
}
}
...
async function showTrendingDetails(alertId) {
    // Could show modal with sample articles
